...
namespace DotNetCoreKoans.Koans
{
    public class AboutGenericContainers : Koan
	{
		...
		public void ArrayListSizeIsDynamic()
		{
			...
			//The size of an array cannot be changed after you allocate it. To get around that                            //ANCA: Wait - you cna get aroudn that????
		...
		public void ArrayListHoldsObjects()
		{
			...
			Assert.Equal(typeof(object), method.GetParameters()[0].ParameterType);  //Anca: run the program and the error tells you the type expected!
		}
		[Step(3)]
		public void MustCastWhenRetrieving()
		{
			...
			int x = 42;
			//x = (int)list[0];
			Assert.Equal(42,x); //ANCA: you need to change x where x is first defined above!
		}
		[Step(4)]
		...
		public void RandomAccess()
		{
			...
			Assert.Equal(7, list[2]); //remember index position starts at 0!!!
		}
		[Step(11)]
		...
		{
			List<int> list = new List<int> { 1, 2, 3 };
            Assert.Equal(new[] { 1, 2, 3 }, list.ToArray()); //ANCA: This took me a bit -- new array [] with the list ...
		}
		[Step(13)]
		...
		public void CatchingMissingData()
		{
			...
			{
				result = "dos"; //ANCA: This doesn't seem very practical - this implies you know the value you are looking for???...
			}
		...
		public void TryGetValueForMissingData()
		{
			...
			Assert.Equal("dos", result); //Anca: There's still not a two in the dictionary, so ...the result is dos...
		}
		[Step(25)]
		...
		public void AddingDataViaSubscript()
		{
			...
			{
				one[item.Key] = item.Value; //ANCA: So this says: for each key-value pair in the two Dictionary list, find the key that exists in the one Dictionary list and reassign it the value from the two dictionary!!!
				//since Amy isn't in the  second list, then her value doens't get reassigned;
				//since Jenny is only in the second list, then her value stays what it is ... BUT DOES JENNY GET ADDED TO THE first list?? one Dictionary???
			}

			Assert.Equal(54, one["jim"]);
			Assert.Equal(26, one["jenny"]); //BUT DOES JENNY GET ADDED TO THE first list ?? one Dictionary ???
